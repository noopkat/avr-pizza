...
module.exports = function requestCompilation(package, callback) {
  ...
  var postRequest = method.request(options, function(response) {
    ...
    response.on('end', function() {
      // done
      return callback(null, JSON.parse(datastring));
    }); 
  });

  postRequest.on('error', function(e) {
    return callback(new Error('everything is sad: '+e));
  }); 
