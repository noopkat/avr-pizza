...
module.exports.requestCompilation = function(package, callback) {
  ...
  switch (os.platform()) {
    case 'darwin': {
      ...
      toolsPath = path.join(builderPath, 'hardware', 'tools');
    ...
    case 'linux': {
      builderPath = path.join(builder.location);
      toolsPath = path.join(builderPath, 'hardware', 'tools');
      libsPath =  path.join(builderPath, 'libraries');
      hardwarePath = path.join(builderPath, 'hardware');
      toolsBuilderPath = path.join(builderPath, 'tools-builder');
  ...
  function compile(callback) {
    ...
    var builderChild = child.exec(path.join(builderPath, 'arduino-builder') + ' ' + builderString, function(error) {
      ...
      fs.readFile(path.join(destDir, path.basename(sketchfile)) + '.hex', function(error, file) {
